...
'''
ORM: Database operation and Model class construct.
'''
...
class ModelMetaclass(type):
    ...
    def __new__(cls, name, bases, attrs):
        ...
        attrs['__update__'] = 'update `{}` set {} where `{}`=?'.format(tableName, ','.join(map(lambda f:'`{}`'.format(mappings.get(f) or f), fields)), primaryKey)
        attrs['__delete__'] = 'delate from `{}` where `{}`=?'.format(tableName, primaryKey)
        return type.__new__(cls, name, bases, attrs)
...
class Model(dict, metaclass=ModelMetaclass):
    ...
    async def findNumber(cls, selectField, where=None, args=None):
        sql = ['select {} __num__ from `{}`'.format(selectField, cls.__table__)]
    ...
    async def save(self):
        ...
        if rows != 1:
            logging.warn("failed to insert record: affected row:{}".format(rows))
    ...
    async def update(self):
        ...
        if rows != 1:
            logging.warn('failed to update record: affected rows:{}'.format(rows))
